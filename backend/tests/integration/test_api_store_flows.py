...
def test_store_item_creation_is_idempotent_by_client_item_id(session: Session) -> None:
    ...
    item = post_store_item(team.id, payload, admin, session)
    repeated_item = post_store_item(team.id, payload, admin, session)
...
def test_store_router_redemption_fulfillment_and_refund_restore_balance_and_stock(
    ...
    fulfilled = post_fulfill_redemption(redemption.id, admin, session)
    assert fulfilled.status == RedemptionStatus.fulfilled

    refunded = post_refund_redemption(redemption.id, admin, session)
...
def test_store_router_fulfillment_notifies_redeeming_user_once(session: Session) -> None:
    ...
    repeat_fulfill = post_fulfill_redemption(redemption.id, admin, session)
...
def test_store_router_unlimited_stock_stays_unlimited_through_refund(session: Session) -> None:
    ...
    post_fulfill_redemption(redemption.id, admin, session)
    refunded = post_refund_redemption(redemption.id, admin, session)
...
def test_store_router_returns_conflict_when_player_balance_is_insufficient(session: Session) -> None:
    team, admin, player = _seed_team(session, player_balance=10)
    item = post_store_item(
        team.id,
...
def test_store_router_returns_conflict_when_stock_is_insufficient_without_charging_coins(
    ...
    team, admin, player = _seed_team(session, player_balance=100)
    item = post_store_item(
        team.id,
...
def test_store_router_preserves_redemption_historical_price_after_item_price_change(
    ...
    patch_store_item(item.id, StoreItemUpdateRequest(price=35), admin, session)
...
def test_store_router_pending_redemption_survives_item_deactivation_and_price_change(
    ...
    patch_store_item(item.id, StoreItemUpdateRequest(price=99, is_active=False), admin, session)
...
def test_inactive_member_cannot_list_or_redeem_store_items(session: Session) -> None:
    team, admin, _ = _seed_team(session, player_balance=100)
...
def test_admin_cannot_create_redemption(session: Session) -> None:
    team, admin, _ = _seed_team(session, player_balance=100)
    item = post_store_item(
        team.id,
        StoreItemCreateRequest(name="管理员不可兑换", price=20, stock=2, is_active=True),
        admin,
        session,
    )
    ...
    assert redeem_error.value.status_code == 403
    assert redeem_error.value.detail["code"] == "STORE_PERMISSION_DENIED"
    assert session.get(StoreItem, item.id).stock == 2
...
def test_member_cannot_redeem_store_item_from_another_team(session: Session) -> None:
    ...
    other_team, other_admin = _seed_second_team(session)
...
def test_store_router_cancel_pending_redemption_restores_stock_once(session: Session) -> None:
    ...
    cancelled = post_cancel_redemption(redemption.id, admin, session)

    assert cancelled.status == RedemptionStatus.cancelled
    assert cancelled.cancelled_by == admin.id
    ...
    second_cancel = post_cancel_redemption(redemption.id, admin, session)
...
def test_store_router_rejects_invalid_redemption_status_transitions_without_side_effects(
    ...
    with pytest.raises(HTTPException) as pending_refund_exc:
        post_refund_redemption(pending_redemption.id, admin, session)
    ...
    fulfilled_redemption = post_fulfill_redemption(pending_redemption.id, admin, session)
    assert fulfilled_redemption.status == RedemptionStatus.fulfilled
    with pytest.raises(HTTPException) as fulfilled_cancel_exc:
        post_cancel_redemption(pending_redemption.id, admin, session)
    ...
    post_cancel_redemption(cancelled_redemption.id, admin, session)
    with pytest.raises(HTTPException) as cancelled_refund_exc:
        post_refund_redemption(cancelled_redemption.id, admin, session)
...
def test_store_router_second_refund_is_idempotent_without_second_refund_transaction(
    ...
    post_fulfill_redemption(redemption.id, admin, session)
    first_refund = post_refund_redemption(redemption.id, admin, session)

    second_refund = post_refund_redemption(redemption.id, admin, session)
...
def test_store_router_lists_support_item_and_redemption_status_filters(session: Session) -> None:
    ...
    post_fulfill_redemption(fulfilled_redemption.id, admin, session)

    assert [item.id for item in read_store_items(team.id, admin, session, True)] == [active_item.id]
    assert [item.id for item in read_store_items(team.id, admin, session, False)] == [inactive_item.id]
    ...
    managed_redemptions = read_team_redemptions(team.id, admin, session, RedemptionStatus.fulfilled)
...
def test_store_router_rejects_cross_team_admin_item_and_redemption_management(
    session: Session,
) -> None:
    team, admin, player = _seed_team(session, player_balance=100)
    _, other_admin = _seed_second_team(session)
    item = post_store_item(
        team.id,
    ...
    with pytest.raises(HTTPException) as patch_item_exc:
        patch_store_item(item.id, StoreItemUpdateRequest(price=1), other_admin, session)
    ...
    with pytest.raises(HTTPException) as manage_redemptions_exc:
        read_team_redemptions(team.id, other_admin, session)
    ...
    with pytest.raises(HTTPException) as fulfill_exc:
        post_fulfill_redemption(redemption.id, other_admin, session)
...
def test_store_refund_transactions_are_database_unique_per_redemption(session: Session) -> None:
    ...
    post_fulfill_redemption(redemption.id, admin, session)
    post_refund_redemption(redemption.id, admin, session)
