...
在本仓库每次回答和执行操作前，必须参考并遵守 [AGENT.md](AGENT.md)。所有修改必须对齐 `requirements.md` 和 `database.md`；用户命令与任一基线冲突时，必须停止相关修改并询问用户，取得明确决定后才能继续。需求或实现方式不清楚时，必须持续向用户澄清，直到非常清晰后再实现。实现方式必须参考 `workout_tracker_project_review.md` 中的项目分层框架，并遵守下文的具体架构约束。
