...
在本仓库每次回答和执行操作前，必须参考并遵守本文件。仓库协作规则与开发约束统一维护在根目录 `AGENTS.md` 中。

### 所有修改必须对齐权威文档

所有修改（包括代码、配置、测试和文档）都必须先核对并对齐以下文件中的相关规定：

- `requirements.md`：产品范围、业务行为、角色和权限。
- `database.md`：数据实体、字段、关系、枚举和数据不变量。
...
执行时必须参考 `workout_tracker_project_review.md` 中的项目分层框架，按以下顺序理解和定位改动：

```text
业务场景 → 数据模型 → 后端分层 → API 契约 → 前端页面与状态
```

沿用现有前后端分离的模块化单体架构，各层职责遵守下文的具体架构约束。

审查文档用于指导理解和分层，其中的示例实体、接口和代码不能替代权威业务、数据及 API 契约；事务等具体技术约束继续遵守本文件和 `tech_stack.md`。不得为了快速实现而绕过、合并或擅自改变现有层级。如本次实现涉及无法消解的文档或架构冲突，必须先停止并询问用户。
...
页面加载遵守 `requirements.md` 的统一状态契约：单轮请求、不自动重试；成功返回空数据只显示空状态，失败不得显示空数据。重试按钮必须根据原始错误类型或 HTTP 状态码决定，不得因存在提示文字或空数组就显示。加载状态与操作反馈分开管理。
